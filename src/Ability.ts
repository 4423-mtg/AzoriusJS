...
class SpellAbility extends Ability {
    instructions: InstructionArray
    constructor(instructions_initializer: () => InstructionArray) {
        super()
        this.instructions = instructions_initializer()
    }
    perform(game_state: GameState, game_history: GameHistory, spell: GameObject) {
        this.instructions.perform(game_state, game_history, spell)
    }
}
